...
urgent$weeks <- recode(urgent$week,
                        ...
                        "27" = "23-27",
                        .default = "NA")
urgent$weeks <- factor(urgent$weeks, 
                        levels = c("6-10", "11-14", 
                                   "15-18", "19-22", "23-27"))

# setup results dataframe 1/2
month_vec <- rep(NA, length(unique(urgent$weeks)) * length(methods))
i <- 1
for(month in sort(unique(urgent$weeks))){
  month_vec[i:(i + length(methods) - 1)] <- rep(month, length(methods))
  i <- i + length(methods)
}

# setup results dataframe 2/2
nrow_res <- length(unique(urgent$weeks)) * length(methods)
method <- rep(methods, length(unique(urgent$weeks)))
mean <- rep(NA, nrow_res)
lower <- rep(NA, nrow_res)
upper <- rep(NA, nrow_res)
res <- data.frame(weeks = month_vec, method, mean, lower, upper)
strats <- data.frame(
  weeks = sort(unique(urgent$weeks)), strata_missing = NA
)
...
for(month in sort(unique(urgent$weeks))){
  ...
  urgent_weeks <- urgent %>% filter(weeks == month)
  n_3 <- nrow(urgent_weeks)
  n_3vec[i] <- n_3
  i <- i + 1
  
  sum_x_delta3 <- sum(urgent_weeks$x)
  rho_hat <- sum_x_delta3 / n_3
  
  # print(head(urgent_weeks))
  # naive results
  ...
  strata_samp <- unique(urgent_weeks[, c(vars_std)]) # nrow = 213
  ...
  urgent_weeks_srg <- left_join(urgent_weeks, tgt_restricted, by = vars_std) %>% 
    select(-c(pat_count)) 
  
  std_ests <- ests_std(urgent_weeks_srg, sigma_e_hat, sigma_p_hat, n_1, n_2, n_3, 
  ...
  ests_srgm <- ests_std_model(urgent_weeks, strata_props, sigma_e_hat,
  ...
  # srgm_formula <- formula("x ~ sex*age*race")
