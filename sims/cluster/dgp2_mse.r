...
for(i in 1:nrow(sim_conditions)){
  ...
  mse_RG <- (hat_pi_RG - row$prev)^2
  mse_SRG <- (hat_pi_SRG - row$prev)^2
  
  sim_conditions[i,"mse_pi_RG"] <- mean(mse_RG[is.finite(mse_RG)])
  sim_conditions[i,"num_infinite_pi_RG"] <- sum(!is.finite(hat_pi_RG))
  sim_conditions[i,"mse_pi_SRG"] <- mean(mse_SRG[is.finite(mse_SRG)])
