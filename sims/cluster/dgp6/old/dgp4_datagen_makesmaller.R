...
source("sim_fns.R")
source("param_values_var_test.R")
...
sim_conditions <- sim_conditions %>% dplyr::select(-c("stratum_props"))
output_filename <- paste("dgp4_datasets_tests/sim_", sim, ".csv", sep = "")
sample_filename <- paste("dgp4_datasets_tests/sample_", sim, ".csv", sep = "")
write_csv(sim_conditions, output_filename)
