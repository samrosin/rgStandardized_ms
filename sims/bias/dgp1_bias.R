...
for(i in 1:nrow(sim_conditions)){
  ...
  hat_pi_RG_notrunc <- rep(NA, n_sims)
  ...
  for(j in 1:n_sims){
    ...
    hat_pi_RG_notrunc[j] <- ests_rg_no_trunc(dat$rho_hat, dat$sigma_e_hat, dat$sigma_p_hat, 
    row$n_1, row$n_2, row$n_3, variance = FALSE)
  }
  ...
  sim_conditions[i,"hat_pi_RG_notrunc"] <- 100 * ( 
    mean(hat_pi_RG_notrunc[is.finite(hat_pi_RG_notrunc)]) - row$prev ) / row$prev 
}
