...
output_file <- here("sims/results_draft/dgp5_var_results.csv")

# The known stratum proportions (the gamma_{zj}s) must be prespecified,
# and they are loaded here
# note dgp3 and dgp5 use same stratum_props
...
sim_conditions <- tidyr::crossing(
  ...
  mutate(prev = sum(stratum_props$stratum_prop * stratum_props$prev),
         ...
         ESE_hat_pi_SRG_restriction = NA_real_, # empirical SE of hat_pi_SRG
         ASE_hat_pi_SRG_restriction = NA_real_, # mean asymptotic SE of hat_pi_SRG
...
for(i in 1:nrow(sim_conditions)){
  ...
  sim_conditions[i,"hat_pi_SRG_restriction"] <- 100 * (
    mean(hat_pi_SRG[!positivity], na.rm = TRUE) - row$prev) / row$prev 
  sim_conditions[i, "ESE_hat_pi_SRG_restriction"] <- sd(hat_pi_SRG[!positivity])
  sim_conditions[i, "ASE_hat_pi_SRG_restriction"] <- mean(sqrt(hat_var_pi_SRG[!positivity]))
  sim_conditions[i, "covers_pi_SRG_restriction"] <- mean(covers_pi_SRG[!positivity])
